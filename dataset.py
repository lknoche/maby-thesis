...
class PositionDataset(Dataset):
    def __init__(
        ...
        if fixed_output_range:
            self.output_range = np.array([0.0, 1.0])
            print(f"[DEBUG] Using fixed output_range: {self.output_range}")
        else:
            logging.warning("Calculating output normalization range")
            self.output_range = da.percentile(
                self.data[:, self.output_channel].flatten(), percentile
            ).compute()
            print(f"[DEBUG] output_range: {self.output_range}")
    ...
    def __getitem__(self, idx):
        ...
        y = (y - self.output_range[0]) / (self.output_range[1] - self.output_range[0])
        x = da.clip(x, 0, 1)
        y = da.clip(y, 0, 1)
...
class LocalPositionDataset(PositionDataset):
    def __init__(
        ...
        super().__init__(
            ...
            start_tp,
            end_tp,
            fixed_output_range=fixed_output_range,
        )
